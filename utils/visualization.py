...
def reconstruction(model, original, use_ddp):
    if use_ddp:
        recons = model.module(original, recon=True)
    else:
        recons = model(original, recon=True)
    grid = []
...
@torch.no_grad()
def pseudo_sampling(model, samples_num, device, use_ddp):
    if use_ddp:
        outs = model.module.pseudo_sampling(samples_num, device=device)
    else:
        outs = model.pseudo_sampling(samples_num, device=device)
    
    grid = make_grid(outs.detach().cpu())
    return grid
